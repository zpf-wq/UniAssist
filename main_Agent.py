...
def make_planner():
    ...
    #           "规则："
    #           "1. **合并无逻辑顺序的任务**：如果多个任务之间没有前后依赖关系，可以合并成一个任务。"
    #             "- 例子：用户说“查上海天气，再查美元汇率”，你可以拆解为："
    #             "- “查询上海天气和美元兑人民币汇率”。"
    #           "2. **功能级别拆解**：只保留可以直接交给代理执行的任务。"
    #             "- ✅ 例子：“查询上海天气”，“查询美元兑人民币汇率”。"
    #             "- ❌ 不要输出“打开天气网站”、“输入上海”、“点击查询”。"
    #           "3. **保持顺序**：如果任务确实有逻辑顺序（前一个结果会影响下一个），就分开拆解。")
    # system = (
    #     "你是一个任务分解器。"
    #     "你的目标是把用户的问题拆解成**最小功能级别的任务**，但不要过度细化为人类的操作步骤。"
    #     "规则："
    #     "1. **合并查询类任务**：如果用户提出多个独立的查询（如查天气、查汇率、查新闻等），没有前后依赖关系，必须合并为一个任务。"
    #     "   - 例子：用户说“查上海天气，再查美元汇率”，你要拆解为："
    #     "     - “查询上海天气和美元兑人民币汇率”。"
    #     "   - 例子：用户说“查北京天气，再查北京大学新闻”，你要拆解为："
    #     "     - “查询北京天气和北京大学新闻”。"
    #     "2. **功能级别拆解**：只保留可以直接交给代理执行的任务。"
    #     "   - ✅ 例子：“查询上海天气和美元兑人民币汇率”。"
    #     "   - ❌ 不要输出“打开网站”、“输入上海”、“点击查询”。"
    #     "3. **保持顺序**：只有在任务确实有前后依赖时才分开拆解；否则全部合并为一个查询任务。"
    # )
    system = (
        "你是一个任务分解器。"
        "你的目标是把用户的问题拆解成**最小功能级别的任务**，但不要过度细化为人类的操作步骤。"
